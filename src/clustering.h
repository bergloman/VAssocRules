...
namespace VikClustering {
    ...
    public:
        ...
        void Finalise(int count) {
            if (count > 0) {
                ...
                if (m_AttrType == VikStd::atDiscr) { InitDiscr(m_Probs.Len()); } else { InitCon(); };
            };
        };
    ...
    public:
        ...
        void Init(VikStd::TAttribute &at, int index, double weight) {
            if (at.AttrType == VikStd::atDiscr) { InitDiscrete(at, index, weight); } else { InitCon(at, index, weight); };
        };
            ...
            if (m_AttrType == VikStd::atDiscr) {
                ...
                if (t >= 0) {
                    for (int i = 0; i < cl.m_Probs.Len(); i++) {
                        if (i == t) { res += SQR(1 - cl.m_Probs[i]); } else { res += SQR(cl.m_Probs[i]); };
                    };
                };
            } else {
                if (tab->Attrs[m_Attr].DoubleIsMissing(ex) == false) {
        ...
        double GetSQRDistance2(TClusterItem &cl, VikStd::TDataTable *tab, int ex) {
            ...
            if (m_AttrType == VikStd::atDiscr) {
                ...
                if (t >= 0) { // if value not missing
                    for (int i = 0; i < cl.m_Probs.Len(); i++) {
                        if (m_ValueIndex[i] == t) { res += SQR(1 - cl.m_Probs[i]); } else { res += SQR(cl.m_Probs[i]); };
                    };
                };
            } else {
                if (tab->Attrs[m_Attr].DoubleIsMissing(ex) == false) {
