...
////////////////////////////////////////////////////////////////////
    ...
    struct TItemset {
        ...
        void PrintF() {
            ...
            for (i = 0; i < m_glbUCharManager.GetItemsetSize(); i++) {
                if (IsSetItem(i)) { m_glbUCharManager.m_Common->Notify("1"); } else { m_glbUCharManager.m_Common->Notify("0"); }
            };
    ...
    struct TIndexNode {
        ...
        void Init(int b, int last) {
            ...
            m_TargetByte = b;
            m_LastByte = last;
            if (b < last) { for (int i = 0; i < 255; i++) m_v.Add(NULL); } else { for (int i = 0; i < 255; i++) m_vb.Add(-1); };
        };
        ...
        void Clear() {
            ...
            if (m_TargetByte < m_LastByte) { for (int i = 0; i < 255; i++) m_v[i] = NULL; } else { for (int i = 0; i < 255; i++) m_vb[i] = -1; };
        };
    };
    ...
    class TListOfItemsets {
        ...
        int GetPivotValN(const int& LValN, const int& RValN) {
            ...
            if (m_v[ValN1] < m_v[ValN2]) {
                if (m_v[ValN2] < m_v[ValN3]) { return ValN2; } else if (m_v[ValN3] < m_v[ValN1]) { return ValN1; } else { return ValN3; };
            } else {
                if (m_v[ValN1] < m_v[ValN3]) { return ValN1; } else if (m_v[ValN3] < m_v[ValN2]) { return ValN2; } else { return ValN3; };
            };
        };
        ...
        int Partition(const int& MnLValN, const int& MxRValN) {
            ...
            do {
                ...
                if (LValN < RValN) { SwapItemsets(m_v[LValN], m_v[RValN]); } else {
        ...
        int SearchBisect(TItemset &Val) const { // find (with bisection) index of given value. -1 if not found.
            ...
            while (l < r) {
                ...
                s = (l + r) / 2;
                if (Val.IsLwr(m_v[s]) != false) { r = s; } else {
                    if (l < s) { l = s; } else { l++; };
                };
            };
            return -1;
