...
namespace VikStd {
	...
	public:
		...
		TStr GetDateAsStr(TStr format){
			...
			TStr res;
			while (i<format.Len()){
				if (format[i]!='%'){ res+=format[i]; }
				else{
					i++;
					if (i<format.Len()){
						if ((format[i]=='d')||(format[i]=='D')){ res+=GetDayOfMonth(); }
						else if ((format[i]=='m')||(format[i]=='M')){ res+=GetMonth(); }
						else if ((format[i]=='y')||(format[i]=='Y')){ 
							if ((GetYear()>=100)&&(GetYear()<110)){ res+="200"; }
							else if (GetYear()>=110){ res+="20"; }
							else res+="19"; 
							res+=GetYear()%100;
						}
		...
		TStr GetTimeAsStr(TStr format){
			...
			while (i<format.Len()){
				if (format[i]!='%'){ res+=format[i]; }
					...
					i++;
					if (i<format.Len()){
						if ((format[i]=='h')||(format[i]=='H')){ res+=GetHour(); }
						else if ((format[i]=='m')||(format[i]=='M')){ res+=GetMin(); }
						else if ((format[i]=='s')||(format[i]=='S')){ res+=GetSec(); }
						else if (format[i]=='%'){ res+='%'; }
					};
				};
				i++;
	...
	inline int ToInteger(char *s, char *Message=NULL){
		...
		while (i<a.Len()){
			if (IsDigit(a.read(i))==true){ r=r*10+(a.read(i)-'0'); i++; }
			else{
				if (Message==NULL){
				...
				while(!feof(fpi)){
					if (c=='<'){
						...
						if (c=='!'){
							...
							if (c=='#'){
								TVar var;
								if (m_Param.Find(GetParamName(),var)==false){ fprintf(fpo,"<!#%s>",m_LastParam.c_str()); }
								else{
